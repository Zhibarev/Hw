...
public:
     ...
     */
    void print() const;
    QVector<QVector<T>> toVector() const;
...
private:
    ...
    void toVector(QVector<QVector<T>> &vector, TreeNode *node, int &position) const;
};
...
typename AVLTree<T>::TreeNode* AVLTree<T>::add(const T &value, TreeNode *node)
{
    ...
    if (value == node->value)
    {
        node->countOfValues = node->countOfValues + 1;
        return node;
    }
    if (value > node->value)
...
typename AVLTree<T>::TreeNode* AVLTree<T>::remove(const T &value, TreeNode *node)
{
    ...
    if (value == node->value)
    {
        ...
        else
        {
            ...
            node->countOfValues = minNode->countOfValues;
            minNode->value = value;
            node->right = remove(value, node->right);
        }
    }
...
template<typename T>
QVector<QVector<T>> AVLTree<T>::toVector() const
{
    QVector<QVector<T>> vector(2, QVector<T>(countOfNodes));
    int position = 0;
...
template<typename T>
void AVLTree<T>::toVector(QVector<QVector<T>> &vector, TreeNode *node, int &position) const
{
    if (node)
    {
        vector[0][position] = node->value;
        vector[1][position] = node->countOfValues;
        position++;
